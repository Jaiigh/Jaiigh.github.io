    ...
    >
      ...
      <div className="grid md:grid-cols-3 gap-12">
        {SKILL_GROUPS.map((group) => (
          <div key={group.label}>
            <div className="anim-header font-mono text-accent2 text-xs uppercase tracking-widest mb-6 pb-3 border-b border-border">
              {group.label}
            </div>
            <div className="flex flex-wrap gap-2">
              {group.skills.map((skill) => (
                <span
                  key={skill}
                  className="anim-tag font-mono text-xs text-foreground border border-border bg-secondary px-3 py-1 hover:border-foreground hover:bg-background transition-colors duration-200 cursor-default"
                >
                  {skill}
                </span>
              ))}
